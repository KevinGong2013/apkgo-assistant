...
3. 面板里是这一家的前置条件和步骤。带「帮我点」的步骤，扩展会替你点到最后一步（打开创建弹窗、填好名称和角色），**确认键永远留给你**；页面一下载密钥文件，面板自动抓到，不用选文件。走到显示密钥的那一页，点「自动识别本页」，或者对每个字段点「点选」再点页面上的值；文件类的密钥（华为 JSON、小米证书、.p8）直接选文件。
