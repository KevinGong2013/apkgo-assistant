...
Chrome / Edge 扩展。打开华为、小米、OPPO、vivo、荣耀、应用宝等开放平台后台时，页面下方出现助手按钮：这六家点一下「一键获取密钥」就拿到 API 密钥并直接保存到 [apkgo cloud](https://apkgo.baici.tech) 验证；其余商店它带你找到密钥那一页，自动识别或点选采集。之后发布就由 apkgo 代你提交到各家商店。
...
| 应用宝 | **一键获取（两段）**：① 在「API发布接口」页调后台接口取 access_secret、从登录信息取开发者 ID；② 你点一下跳到应用列表页，助手取全部应用的包名与 App ID → 自动保存验证 | ✅ 真实后台跑通 |
...
一键获取的六家（华为、小米、OPPO、vivo、荣耀、应用宝）都在真实后台跑通过。一键获取前会先检查登录状态，没登录会明确提示。「未实登核对」的商店，面板里的菜单名和入口 URL 来自官方文档与社区教程，改版了对不上就改 [`src/recipes.js`](src/recipes.js)，一家商店就是一个对象；补上 `flow` 和 `actions` 就能变成一键。
