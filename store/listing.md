...
• 打开华为、小米、OPPO、vivo、荣耀、应用宝、魅族、Samsung、App Store 的开发者后台时，页面下方出现助手按钮，面板里是这家的前置条件和该点的菜单。
• 华为、小米、OPPO、vivo、荣耀、应用宝支持「一键获取密钥」：小米、OPPO、vivo、荣耀直接通过后台自己的接口读出凭据并保存验证，几秒完成；华为由扩展跳到密钥页、打开创建弹窗并填好，你只需点一下「确认」，下载的密钥文件自动抓取、保存、验证。
• 走到显示密钥的那一页，点「自动识别本页」，或者对每个字段点「点选」再点页面上的值；文件类密钥直接选文件。
