...
>
> 原来的自动发布工作流 `publish.yml` 已删除：一是不再上架，二是它本身就是坏的——
> 步骤里用了 `if: ${{ secrets.X != '' }}`，而 `secrets` 上下文在 `if` 表达式里不可用，
> GitHub 判定为无效工作流文件，导致每次 push 都记一次失败。将来若要恢复，改用
> `env:` 先把 secret 映射成环境变量，再在脚本里判断是否为空。
